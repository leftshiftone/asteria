...
import static one.leftshift.asteria.report.AsteriaReportPlugin.DEPS_GRAPH_TASK_NAME

class DepsGraphTask extends DefaultTask {
    ...
    DepsGraphTask() {
        ...
        def fileDir = new File("${project.buildDir}/report/${DEPS_GRAPH_TASK_NAME}")
        if (!fileDir.exists()) {
            fileDir.mkdirs()
        }
